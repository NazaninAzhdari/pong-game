...
architecture RTL of pong_paddle is
    ...
    signal r_paddle_y_top       :   integer range 0 to (pc_GAME_HEIGHT - pc_PADDLE_HEIGHT -1)  :=11;
    signal r_paddle_y_dwn       :   integer range (pc_PADDLE_HEIGHT -1) to (pc_GAME_HEIGHT-1)  :=17; 
    ...
    begin
        ...
        process(i_clk) is
                ...
                if rising_edge(i_clk) then
                    if r_paddle_move_count < pc_PADDLE_SPEED-1 then
                        ...
                        r_paddle_move_count <= 0;
                            ...
                            if r_btn_DV = '1' then
                                if i_btn_up = '1' and r_y_paddle_top /=0 then
                                    r_y_paddle_top <= r_y_paddle_top + 1;
                                    r_y_paddle_dwn <= r_y_paddle_dwn -1;
                                end if;

                                if i_btn_dwn = '1' and r_y_paddle_dwn /= pc_GAME_HEIGHT-1 then
                                    r_y_paddle_top <= r_y_paddle_top - 1;
                                    r_y_paddle_dwn <= r_y_paddle_dwn + 1;
                                end if;
                            end if;
            ...
            o_draw_paddle <= '1' when (r_x < g_X_LOCATION_PADDLE + pc_PADDLE_SIZE 
                                        and r_x >= g_X_LOCATION_PADDLE 
                                        and r_y < r_paddle_y_btn 
                                        and r_y > r_paddle_y_top)
                                        else '0';
